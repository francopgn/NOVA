...
import { Plus, ChevronUp, ChevronDown, Copy, Pencil, Home, Search, ListTree, ListChecks } from "lucide-react";
...
export default function AdminCategoriesPage() {
  ...
  const { fieldsFor } = useCustomFields();
              ...
              return (
                <div key={cat.id} className={`flex flex-wrap items-center gap-4 rounded-2xl border bg-card p-4 ${!cat.active ? "opacity-50" : "border-border"}`}>
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <Link href={`/admin/categorias/${cat.id}/campos`}>
                      <Button size="icon-sm" variant="ghost" aria-label="Campos personalizados" title={`${fieldsFor(cat.id).length} campos`}>
                        <ListChecks size={14} />
                      </Button>
                    </Link>

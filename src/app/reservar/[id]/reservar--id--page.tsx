...
import { Check, ChevronLeft, PartyPopper, Video, Building2, Home as HomeIcon, Building, Plane, CreditCard, Landmark } from "lucide-react";
...
export default function BookingPage({ params }: { params: { id: string } }) {
  ...
  const [paymentMethod, setPaymentMethod] = React.useState<"tarjeta" | "transferencia" | null>(null);
  ...
  React.useEffect(() => {
    if (!professional || paymentMethod) return;
    if (professional.acceptsCards && !professional.acceptsTransfer) setPaymentMethod("tarjeta");
    else if (!professional.acceptsCards && professional.acceptsTransfer) setPaymentMethod("transferencia");
  }, [professional, paymentMethod]);
              ...
              {step === 5 && (
                <div>
                  ...
                  <div className="mb-5 grid grid-cols-1 gap-3 sm:grid-cols-2">
                    {professional.acceptsCards && (
                      <button
                        onClick={() => setPaymentMethod("tarjeta")}
                        className={cn(
                          "flex items-center gap-3 rounded-2xl border p-4 text-left transition-colors",
                          paymentMethod === "tarjeta" ? "border-primary bg-primary/15" : "border-border hover:bg-white/5"
                        )}
                      >
                        ...
                        <span>
                          ...
                          <span className="block text-xs text-muted-foreground">•••• •••• •••• 4242</span>
                        </span>
                      </button>
                    )}
                    {professional.acceptsTransfer && (
                      <button
                        onClick={() => setPaymentMethod("transferencia")}
                        className={cn(
                          "flex items-center gap-3 rounded-2xl border p-4 text-left transition-colors",
                          paymentMethod === "transferencia" ? "border-primary bg-primary/15" : "border-border hover:bg-white/5"
                        )}
                      >
                        <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-primary/15 text-primary">
                          <Landmark size={17} />
                        ...
                        <span>
                          ...
                          <span className="block text-xs text-muted-foreground">Alias: sessio.pagos</span>
                        </span>
                      </button>
                    )}
                  </div>
                  ...
                  <div className="flex flex-col divide-y divide-border rounded-2xl border border-border">
                    ...
                    <SummaryRow
                      ...
                      value={paymentMethod === "tarjeta" ? "Tarjeta •••• 4242" : paymentMethod === "transferencia" ? "Transferencia bancaria" : "—"}
                    />
